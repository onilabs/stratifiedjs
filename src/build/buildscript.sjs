...
function build_deps() {
  ...
  //----------------------------------------------------------------------
  // testsuite index
  BUILD("testsuite/_index.txt",
      [
      "echo -e '### NOTE: do not edit this file manually,\n### regenerate it by running src/build/make-apollo\n' > $TARGET",
      "find testsuite -name '*-tests.sjs' | sort | cut -d / -f 2- >> $TARGET",
      ]
      );
  PSEUDO("testsuite/_index.txt"); // it's backed by a file, but its dependencies are not enumerable
